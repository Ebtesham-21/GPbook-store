  ...
  useEffect(() => {
    const fetchSliders = async () => {
      try {
        ...
        console.error("Error fetching sliders:", error);  // More detailed error logging
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setSliders([...sliders, result.slider]); // Update the UI
      } else {
        ...
        {sliders.map((slider) => (
          <div key={slider._id} className="border p-4">
            ...
            <div className="mt-2 space-x-2">
              <button onClick={() => handleEdit(slider._id)} className="bg-blue-500 text-white px-4 py-2 rounded">
                Edit
              </button>
